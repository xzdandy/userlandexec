...
save_argv(int argc, char **argv)
{
	...
	int i, len;
	char *str;
    const char *libpreload = "LD_PRELOAD=./idea/libexitpreload.so";
    const char *piperead = "PIPE_READ=./pip.tmp";
	...
	if (argc > 0)
        ...
        len += strlen(libpreload) + 1;
        ++argc;
        len += strlen(piperead) + 1;
        ++argc;
	}
	...
	for (i = 0, str = r->block; i < argc; i++)
	{
        ...
        {
            for (j = 0; libpreload[j]; ++j)
                str[j] = libpreload[j];
            str[j] = '\0';
            str += (j + 1);
            for (j = 0; piperead[j]; ++ j)
                str[j] = piperead[j];
            str[j] = '\0';
            str += (j + 1);
            break;
